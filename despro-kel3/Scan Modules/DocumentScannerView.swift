...
struct DocumentScannerView: UIViewControllerRepresentable {
    ...
    class CustomCameraViewController: UIViewController, AVCapturePhotoCaptureDelegate {
        ...
        @objc func saveAndDismiss() {
            ...
            coordinator?.bleManager.writeValue("5");
        ...
        private func observeBleConnection() {
            ...
            cancellable = bleManager.$isConnected
                .sink { [weak self] newValue in
                    // This block will execute when receivedValue changes
                    self?.handleBleConnectionChange(newValue)
                }
        }
        
        func handleBleConnectionChange(_ newValue: Bool) {

            print("📲 BLE STATUS changed: \(newValue)")            
            // Trigger photo capture if the value matches a condition
            if newValue == false {
                coordinator?.bleManager.connect()
            }
        }
        
        
        
        private func observeReceivedValues() {
            // Check if coordinator and bleManager are available
            guard let bleManager = coordinator?.bleManager else { return }

            // Combine the two publishers
            cancellable = Publishers.CombineLatest(bleManager.$receivedValue, bleManager.$isConnected)
                .sink { [weak self] receivedValue, isConnected in
                    // Handle changes from both variables
                    self?.handleReceivedValuesChange(receivedValue, isConnected)
                }
        }

        func handleReceivedValuesChange(_ receivedValue: String, _ isConnected: Bool) {
            // Check if the device is connected
            if !isConnected {
                print("BT Disconnected")
                coordinator?.bleManager.connect() // Attempt to reconnect
                return // Early exit if not connected
            }

            // If receivedValue is empty, skip the action
            guard !receivedValue.isEmpty else {
                print("⚠️ Received value is empty, skipping action.")
                return
            }

            // Trigger photo capture if the value matches a condition
            if receivedValue == "CAPTURING" {
                capturePhoto()
            } else if receivedValue == "DONE" {
                saveAndDismiss()
            }
        }
        ...
        @objc func capturePhoto() {
            ...
            coordinator?.bleManager.writeValue("4")
        }
